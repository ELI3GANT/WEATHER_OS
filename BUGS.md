...
| WOS-014 | P2 | iOS TestFlight beta pipeline | Attempt to distribute the new beta scheme before Apple portal setup. | No beta App ID, App Store Connect record, or Xcode Cloud workflow can be verified from the repository/Linux host. | Repository now supplies `WeatherOS Beta` / `tech.onlytrueperspective.weatheros.beta`; create and bind the Apple-side records without changing production. | XML, shell, project-reference, and Dart analysis checks pass; cloud archive pending. | Blocked — Apple portal actions |
